...
if(!empty($_POST['nombre']) && !empty($_POST['password']) && !empty($_POST['email'])){
    ...
    $solicitud = "INSERT INTO usuarios(`Usuario`, `Password`,`Correo`) VALUES('$nombre', '$hash', '$email')";
    ...
    try {
        if(!mysqli_query($conexion, $solicitud))
        {
         ...
         echo " <!DOCTYPE html>
           ...
           <p class='h4'>El nombre de usuario o el correo ya existen</p>
           </div>
            <div class='text-center'>
            <a type='button' class='btn btn-primary btn-lg mt-4 col-12' href='../views/UsuarioRegistro.php'>Volver a intentar</a>
            </div>
    ...
    header("location: ../views/UsuarioCreado.php");
}
